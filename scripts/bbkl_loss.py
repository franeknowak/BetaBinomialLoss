...
def beta_binomial_nll_per_sample(alpha: torch.Tensor,
    ...
    """
    Per-sample Beta–Binomial NLL for one head / one class.
    alpha: (B,2) with alpha[:,0]=a0 (neg), alpha[:,1]=a1 (pos)
    y_soft: (B,) or (B,1) in {0,1/3,2/3,1}
    returns: (B,) nll
    """
    a0 = alpha[:, 0]
    a1 = alpha[:, 1]
    ...
    nf = torch.tensor(float(n_annotators), device=alpha.device, dtype=a0.dtype)
...
def dirichlet_kl_per_sample(alpha: torch.Tensor,
    ...
    """
    Per-sample KL(Dir(alpha) || Dir(prior_alpha)) for 2D Dirichlet (== Beta case).
    alpha: (B,2)
    prior_alpha: None -> Dir(1,1); or shape (2,) or (B,2)
    returns: (B,) kl
    """
    ...
    return torch.distributions.kl.kl_divergence(posterior, prior)  # (B,)
...
def evidential_bb_loss( alpha: torch.Tensor,
    ...
    # Build per-sample weights based on majority vote (k >= 2 for n=3)
